...
if (accessByToken($conn, $token, $publicKey)) {
    $actionsqlstmt = $conn->prepare("INSERT INTO `iotdevices`(`id`, `iotname`, `devicetype`, `gpionumber`, `devicedesc`, `users`) VALUES (NULL,?,?,?,?,?)");
    $actionsqlstmt->bind_param("ssiss", $iotname, $devicetype, $gpionumber, $devicedesc, $username);
    $username = getUsername($conn, $token, $publicKey);
    $result = $actionsqlstmt->execute();

    if ($result) {
        echo json_encode(array("success" => true, 'newid' => $conn->insert_id));
    } else {
